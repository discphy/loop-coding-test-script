...
2. 결과만 반환 (저장하지 않음)
3. 제출자/미제출자 목록 표시 (@멘션 포함)
4. Slack ephemeral 메시지로 본인에게만 표시

**실제 집계 모드** (날짜 파라미터 있음):
1. 사전 검증 실행 (챌린저 존재, 중복 집계, 날짜 유효성)
2. 검증 통과 시 즉시 "집계를 시작합니다..." 응답 (3초 타임아웃 방지)
3. 백그라운드에서 집계 실행:
   - 지정한 날짜의 집계 데이터 계산
   - `aggregation` 시트에 저장
   - `challenger-stats` 시트 업데이트 (경고 누적 포함)
4. Slack 일반 채널에 결과 전송 (2개 메시지):
   - 간단 요약: "오늘 하루도 고생하셨습니다. X명 챌린지 완료 ✅, Y명 챌린지 실패 ❌"
   - 상세 결과: 챌린저 수, 성공률, 제출자/미제출자 @멘션 목록
5. 경고 초과 시 관리자 채널 알림

**권한:**
- `ADMIN_CHANNEL_ID`가 설정된 경우: 해당 채널에서만 사용 가능
- `ADMIN_CHANNEL_ID`가 미설정된 경우: 모든 채널에서 사용 가능

**정책:**
- **중복 집계 방지** - 이미 집계된 날짜는 재집계 불가
- **날짜 형식 검증** - `yyyy-MM-dd` 형식만 허용
- **과거 날짜만 허용** - 오늘 날짜 이전만 집계 가능
- **Slack 멘션 지원** - 제출자/미제출자 @멘션으로 표시
- **타임아웃 방지** - 즉시 응답 후 백그라운드 처리

**응답 메시지:**

조회 모드 (ephemeral - 본인만 보임):
```
📊 2025-10-23 집계 조회
👥 총 챌린저: 10명
✅ 제출 성공: 8명
...
실제 집계 모드 - 즉시 응답 (ephemeral - 본인만 보임):
```
📊 2025-10-22 집계를 시작합니다...
잠시 후 채널에 결과가 전송됩니다.
```

실제 집계 모드 - Slack 채널 메시지 (전체 공개):
```
...
- 챌린저 없음: `⚠️ 집계 대상 챌린저가 없습니다.`
- 날짜 형식 오류: `❌ 잘못된 날짜 형식입니다.\n올바른 형식: yyyy-MM-dd (예: 2025-10-22)`
- 미래 날짜: `❌ 2025-10-25은(는) 집계할 수 없습니다.\n오늘 날짜 이전만 집계 가능합니다.`
